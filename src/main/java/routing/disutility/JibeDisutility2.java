...
public class JibeDisutility2 implements TravelDisutility {
    ...
    private final double mcLightingVgvi;
    private final double mcShannon;
    private final double mcStressLink;
    private final double mcStressJct;
    private final TravelTime timeCalculator;

    // Custom parameters
    public JibeDisutility2(String mode, TravelTime tt,
                           double mcTime_s, double mcDistance_m,
                           double mcGradient_m_100m, double mcShannon, double mcLightingVgvi,
        ...
        this.mcShannon = mcShannon;
        this.mcLightingVgvi = mcLightingVgvi;
    ...
    private void printMarginalCosts() {
        logger.info("Initialised JIBE disutility with the following parameters:" +
                ...
                "\nMarginal cost of shannon (/m): " + this.mcShannon +
                "\nMarginal cost of lighting/vgvi (/m): " + this.mcLightingVgvi +
    ...
    public double getLinkTravelDisutility(Link link, double time, Person person, Vehicle vehicle) {
        ...
        if(link.getAllowedModes().contains(this.mode)) {
            ...
            // Shannon
            double shannon = LinkAmbience.getShannonFactor(link);
            disutility += mcShannon * shannon * distance;

            // Lighting / VGVI
            double lightVgvi;
            if(time < 21600 || time >= 72000) {
                // Nighttime - use Lighting
                lightVgvi = LinkAmbience.getLightingFactor(link);
            } else {
                // Daytime - use VGVI
                lightVgvi = LinkAmbience.getVgviFactor(link);
            }
            disutility += mcLightingVgvi * lightVgvi * distance;
